// Access the segment controls exposed by libpf's CustomPipeline. They are not forwarded
// by StarfishMediaAPIs, and libpf ships no headers in the NDK - only a pkg-config file
// pointing at a directory that does not exist - so its two entry points are reached by
// mangled name through dlsym.
//
// The Player pointer itself comes from the C++ shim (smp_player()), which reads the
// public `player` member of the instance. The offset arithmetic this file used to do is
// gone with it.
//
// boost::shared_ptr is two pointers. getPipeline() returns a temporary owner; its extra
// reference is dropped below with the same atomic operation Boost's inline shared_count
// destructor uses.
#include <dlfcn.h>
#include <stdbool.h>
#include <stddef.h>
...
bool jf_starfish_begin_segment(int64_t pts_ns)
{
    ...
    symbols.getPipeline(&pipeline_owner, player);
    // Keep the non-owning pointer. Starfish's player holds the lifetime for the whole
    // serialized call and for the subsequent playback session.
    ...
    if (!release_pipeline_copy(&pipeline_owner, &refs_before, &refs_after)) {
        fprintf(stderr, "Starfish segment: unexpected shared_ptr refcount=%u\n", refs_before);
    ...
    if (ok) {
        ...
        // The JSON key is misspelled by libpf ("seperatedPTS") and is not reflected in
        // every firmware's loadSpi_getInfo result. A raw ES feed needs it in the segment
        // descriptor itself for the fed timestamps to be taken as presentation times
        // rather than being re-derived from the segment.
        info.separatedPTS = 1;
